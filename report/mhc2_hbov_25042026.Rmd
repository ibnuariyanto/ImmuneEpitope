---
title: "MHC-II Multi-Method Binding Analysis for Vaccine Epitope Selection"
...
This report performs a **multi-method** in-silico MHC-II binding analysis
for vaccine epitope selection. Three IEDB-recommended Class II methods are
integrated:

1. **NetMHCIIpan EL** - eluted-ligand percentile (primary IEDB-recommended)
2. **NetMHCIIpan BA** - binding-affinity percentile (secondary, shown if present)
3. **SMM_align**     - stabilized matrix peptide-MHC alignment
4. **Sturniolo**     - TEPITOPE matrix (DR alleles only)

Lower percentile rank = stronger predicted binder (uniform across methods).
Thresholds: **<= 2%** strong | **<= 10%** moderate.
A peptide is a **consensus binder** if it crosses the threshold in **>= 2**
of the available methods for at least one allele.
...
# Import data & detect available methods
```{r import}
...
# IMPORTANT: IEDB Class II uses different score columns per method.
#   NetMHCIIpan_EL / BA  -> column family ".percentile."
#   SMM_align            -> column family ".adjusted_percentile." only
#   Sturniolo            -> column family is named "tepitope" with ".adjusted_percentile."
# The patterns below accept both ".percentile." and ".adjusted_percentile."
# so a single registry works across all four IEDB Class II methods.
...
# Keep only methods that actually appear in df_wide
...
cat("Methods detected in consolidated table:\n")
print(method_registry %>% select(method, n_cols))

# Safe references for the global / median columns and the long table
median_col     <- grep("overall|global.*median|median.*global", colnames(df_wide), value = TRUE)[1]
core_col       <- grep("core",       colnames(df_long), value = TRUE)[1]
percentile_col <- grep("percentile", colnames(df_long), value = TRUE)[1]
method_col_long<- grep("method",     colnames(df_long), value = TRUE)[1]  # may be NA if file is single-method
...
# Tidy wide → long (method × allele × peptide)
```{r tidy}
...
#  Cross-method consensus shortlist (promiscuity-based)
```{r consensus-binders}
...
# Faceted heatmaps (one per method)

Narrative: `# Binding Heatmaps - faceted by method`
...
# Promiscuity per method + combined

Narrative: `# Promiscuous Binder Selection`
```{r promiscuous-table}
...
# Positional distribution, colored by method
```{r pos-dist}
if (!is.na(method_col_long)) {
  pos_df <- df_long %>%
...
# k-means PCA on consensus matrix
```{r kmeans-consensus}
set.seed(42)
k_chosen <- 4
...
# Cluster summary across methods
```{r cluster-summary}
...
# Final multi-method shortlist

## Final Vaccine Candidate Shortlist (multi-method)
```{r shortlist}
...
# Defensive: rebuild `consensus` if Chunk 5b wasn't sourced this session OR
# the object has the old schema (missing `total_allele_hits`/`best_min_pct`).
required_cols <- c("methods_pass", "total_allele_hits", "best_min_pct")
if (!exists("consensus") || !all(required_cols %in% names(consensus))) {
  message("`consensus` missing or stale schema - rebuilding inline ",
          "(re-source Chunk 5b for tunable params).")
...
install.packages(
  c("tidyverse", "readr", "pheatmap",
    "factoextra", "ggrepel", "ggseqlogo",
    "knitr", "kableExtra", "rmarkdown",
    "cowplot", "corrplot", "patchwork"),
